...
import os
import re
from headphones import logger, searcher, pg as db , importer, mb, lastfm, librarysync, helpers, notifiers, crier
...
class WebInterface(object):
    ...
    def home(self):
        ...
        artists = myDB.select('SELECT * from artists order by ArtistSortName')
    ...
    def getArtists_json(self, iDisplayStart=0, iDisplayLength=100, sSearch="", iSortCol_0='0',
        ...
        if sSearch == "":
            query = 'SELECT * from artists order by %s %s' % (sortcolumn, sSortDir_0)
            filtered = myDB.select(query)
            totalcount = len(filtered)
        else:
            query = 'SELECT * from artists WHERE ArtistSortName LIKE "%' + sSearch + '%" OR LatestAlbum LIKE "%' + sSearch + '%"' + 'ORDER BY %s %s' % (
